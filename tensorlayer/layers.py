...
class GroupNormLayer(Layer):
    ...
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            self.inputs = tf.reshape(self.inputs,[N, H, W, C // G, G])
            
            mean, var = tf.nn.moments(self.inputs,[1, 2, 3], keep_dims=True)
            self.inputs = (self.inputs - mean) / tf.sqrt(var + epsilon)
            self.inputs = tf.reshape(self.inputs,[N, H, W, C])
            self.outputs = self.inputs * gamma + beta
            self.outputs = act(self.outputs)
